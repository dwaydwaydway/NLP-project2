...
TrainDataPath = "Data/trainset-levela.tsv"
TestDataPath = "Data/testset-levela.tsv"
ValidationRatio = 0.99
...
OutputDir = "ModelA"
ResultDir = "ResultA"
# LoadModel = "ModelWeightA_01.model"
# LoadConfig = "ModelConfigA_01.config"
SaveModel = "f1_ModelWeightA_" + \
    str(NUMofEPOCH) + "_" + str(MAXSEQLEN) + ".model"
SaveConfig = "f1_ModelConfigA_" + \
    str(NUMofEPOCH) + "_" + str(MAXSEQLEN) + ".config"
SaveSubmission = "f1_SubmissionA_" + \
...
# 	Train, Validation = train_test_split(Train, test_size=ValidationRatio, random_state=RandomState)
    ...
    TrainExamples = [InputExample('Train', row.tweet, label=row.subtask)
                     for row in Train.itertuples()]
    # ValidationExamples = [InputExample('Val', row.tweet, label=row.subtask) for row in Validation.itertuples()]
    TestExamples = [InputExample('Test', row.tweet, label=LabelList[0])
    ...
    for epoch in trange(int(num_train_epochs), desc="Epoch"):
        ...
        # Model.load_state_dict(torch.load(output_model_fi

        # Run Test
        res = predict(Model, tokenizer, TestExamples,
                      LabelList, max_seq_length)
