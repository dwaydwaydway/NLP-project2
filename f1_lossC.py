...
SaveModel = "f1_ModelWeightC_" + str(NUMofEPOCH) + "_" + str(MAXSEQLEN) + ".model"
SaveConfig = "f1_ModelConfigC_" + str(NUMofEPOCH) + "_" + str(MAXSEQLEN) + ".config"
SaveSubmission = "f1_SubmissionC_" + str(NUMofEPOCH) + "_" + str(MAXSEQLEN) + ".csv"
...
#             loss_fct = CrossEntropyLoss()            
            loss = custom_loss(logits.view(-1, self.num_labels), labels.view(-1))
            return loss
...
if __name__ == "__main__":
	...
	Model = F1_BertForSequenceClassification.from_pretrained(BERTModel, cache_dir=cache_dir, num_labels=len(LabelList))
	Model.to(device)
	if n_gpu > 1:
	...
	optimizer_grouped_parameters = [
		...
		{'params': Model.classifier.parameters(), 'weight_decay': 0.0,'lr': classifierLR}
		]
...
# 				print("Fininshed: {:.2f}% ({}/{})".format(step/total_step*100, step, total_step), end='\r')
